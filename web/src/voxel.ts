...
export function createDefaultEngine(canvas: HTMLElement) {
...
function CreateChunk(chunkOffset, scene, mat, 
  chunkVoxels = [], mesh = new BABYLON.Mesh(`chunk_${chunkOffset.x}_${chunkOffset.z}`, scene), 
  vertexData = new BABYLON.VertexData(), update = false) {
...
export function runGame(canvas: HTMLElement) {
  try {
