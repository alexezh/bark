...
export class Readline implements ITerminalAddon {
  ...
  private onDataToken?: IDisposable;
  private onDataChar: boolean = false;
  ...
  public activate(term: Terminal): void {
    this.term = term;
    this.onDataToken = this.term.onData(this.readData.bind(this));
   ...
   */
  public read(prompt: string, suppressHistory: boolean = false): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      this.state.refresh();
      this.activeRead = { prompt, resolve, reject, suppressHistory };
    });
  }
  ...
  public readChar(prompt: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      this.state.refresh();
      this.activeRead = { prompt, resolve, reject, suppressHistory: true };
    });
  }

  public cancelRead() {
    this.activeRead?.reject('read cancelled');
  ...
  private updateOnData(onDataChar: boolean) {
    if (this.term === undefined) {
      return;
    }

    if (onDataChar === this.onDataChar) {
      return;
    }

    if (this.onDataToken !== undefined) {
      this.onDataToken.dispose();
      this.onDataToken = undefined;
    }

    if (onDataChar) {
      this.onDataToken = this.term.onData(this.readDataChar.bind(this));
    } else {
      this.onDataToken = this.term.onData(this.readData.bind(this));
    }

    this.onDataChar = onDataChar;
  }

  private readDataChar(data: string) {
    const input = parseInput(data);
    if (input.length == 0) {
      return;
    }

    if (input[0].inputType !== InputType.Text) {
      return;
    }

    this.state.editInsert(input[0].data.join(""));
    ...
    switch (input.inputType) {
      ...
      case InputType.Enter:
        if (this.checkHandler(this.state.buffer())) {
          this.state.moveCursorToEnd();
          this.term?.write("\r\n");
          if (!this.activeRead.suppressHistory) {
